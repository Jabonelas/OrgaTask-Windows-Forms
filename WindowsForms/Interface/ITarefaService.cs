...
namespace WindowsForms.Interface
{
    public interface ITarefaService
    {
        ...
        Task<ResultadoOperacao> CadastrarTarefaAsync(TarefaAlterarDTO _dadosTarefa);

        Task<(ResultadoOperacao ResultadoOperacao, TarefaAlterarDTO Tarefa)> BuscarTarefaAsync(int _idTarefa);

        Task<(ResultadoOperacao ResultadoOperacao, TarefaQtdStatusDTO TarefaQtdStatusDTO)> BuscarQtdStatusTarefaAsync();

        Task<(ResultadoOperacao ResultadoOperacao, List<TarefaPrioridadeAltaDTO> ListaTarefaPrioridade)> BuscarTarefasPrioridadeAltaAsync();
    }
}